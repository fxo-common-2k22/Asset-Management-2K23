$(document).on('change', '.getItemCodes', function (event) {
    var that = this;
    $(that).closest('tr').find("[id $= PIPDetailId]").empty().change();
    if ($(that).val()) {
        $.ajax({
            url: "/AM/ReturnIssue/GetItemCodesById/" + $(that).val(),
            type: "Get",
...
function binditemcodes(data, that) {
    if (data.codes.length > 0) {
        $(that).closest('tr').find("[id $= PIPDetailId]").empty();
        $.each(data.codes, function (i, value) {
            $(that).closest('tr').find('[id $= PIPDetailId]').append("<option data-qty=" + value.Qty + " value= " + value.Value + ">" + value.Text + "</option>");
        });
        if (data.IsConsumable && data.IsConsumable == true) {
            ...
            $(that).closest('tr').find('[id $= PIPDetailId]').prop('readonly', false);
            $(that).closest('tr').find('[id $= PIPDetailId]').prop({ disabled: false });
        } else {
            $(that).closest('tr').find('[id $= PIPDetailId]').prop('readonly', false);
            $(that).closest('tr').find('[id $= PIPDetailId]').prop({ disabled: false });
            $(that).closest('tr').find("[id $= Quantity]").val(1);
...
function rowReset(that) {
    ...
    $(that).closest('tr').find("[id $= Quantity]").prop('readonly', false);
    $(that).closest('tr').find("[id $= Quantity]").prop('disabled', false);
    $(that).closest('tr').find("[id $= PIPDetailId]").prop('readonly', true);
    $(that).closest('tr').find("[id $= PIPDetailId]").prop({ disabled: true });
}
...
$(document).on('keydown', '.barcodeField', function (e) {
    if (e.which == 13) {//Enter key pressed
        ...
        if ($(this).val()) {
            $.ajax({
                url: '/AM/ReturnIssue/GetItemByBarcode',
                type: 'post',
                data: { id: $(this).val() },
                success: function (data) {
                    if (data) {
...
$("#ReturnIssue_ReturnIssueId").keydown(function (e) {
    e.preventDefault();
    if (e.keyCode == 40)//arrow down
    {
        $("#Description").focus();
    ...
    if (e.keyCode == 39)//right arrow key
    {
        ...
        $("#ReturnIssue_ReturnIssueDate").focus();
    }
})
$("#ReturnIssue_ReturnIssueDate").keydown(function (e) {
    if (e.keyCode == 40)//arrow down
    {
        e.preventDefault();
        $("#Description").focus();
    ...
    if (e.keyCode == 37)//arrow left
    {
        ...
        $("#ReturnIssue_ReturnIssueId").focus();
    }
    if (e.keyCode == 39)//right arrow key
    {
        e.preventDefault();
...
$(document).on('keydown', '#ReturnIssue_ReturnIssueId', function () {
    //$('#ReturnIssue_ReturnIssueId').keydown(function (e) {
    if (e.which == 13) {//Enter key pressed
        e.preventDefault();
        window.location = '/AM/ReturnIssue/AddEditIssuedItem/' + this.value;
    }
});

function IsValid() {
    var validated = true;
    if (!$('#ReturnIssue_ReturnIssueDate').val()) {
        $('#ReturnIssue_ReturnIssueDate').focus();
...
function PrintInvoice() {
    if ($('#ReturnIssue_ReturnIssueId').val() > 0) {
        $('#mainsection').hide();
        $('#printsection').show();
