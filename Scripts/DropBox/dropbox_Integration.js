...
function CreateNewAttachment(event) {
    $.ajax({
        url: '/Academics/LessonPlan/CreateResourceAttachments',
        type: 'POST',
        ...
        error: function (el) {
            ...
            $("#photosgrid").load("/Academics/LessonPlan/GetAttachments" + pid);
            DeActivateFileLoader();
        },
        statusCode: null,
        onProgress: null,
...
function SaveDownloadAbleLinkToDatabase(DownloadableLink, fileId, attachmentId) {
    $.ajax({
        url: '/Academics/LessonPlan/SaveDropboxAttachmentsToDatabase',
        type: 'POST',
